...
def initialize_train__regressor(X_train, y_train, **kwargs):
    logger.info(f"Initializing and training VAE Regressor...")
    
    model = None

    return model

def VAE_nll(xgboost_regressor_model, X_test, y_test):
    """
    Calculates the model-based Negative Log-Likelihood (NLL) for a fitted VAE,
...
def evaluate_VAE_model(model, X_test, y_test, y_pred):
    """
    ...
    avg_nll = VAE_nll(model, X_test, y_test)

    logger.info(f"VAE_regressor Model Test Mean NLL: {avg_nll:.4f}")
...
def run_VAE_pipeline(
    ...
    test_datasets = None,
    base_model_save_path_template : str = None # "trained_models/VAE_best_{dataset_key}.pth"
    ):
    """
    ...
    for dataset_key, dataset_info_dict in datasets_to_run.items():
        ...
        logger.info(f"===== Starting VAE {num_folds_to_run}-Fold Evaluation for: {dataset_name} ({dataset_key}) =====")
        ...
        for fold_idx in range(num_folds_to_run):
            ...
            X_train, y_train, X_test, y_test = \
                load_preprocessed_data("VAE", source_dataset, dataset_key, fold_idx,
            ...
            model_xgb = initialize_train__regressor(X_train, y_train, **xgboost_params)
           ...
           # to fix bug of diffrenet devices
            ...
            avg_nll, reg_metrics = evaluate_VAE_model(
...
def run_VAE_optuna(
    ...
    for dataset_key in datasets_to_optimize:
        ...
        X_train, y_train, X_test, y_test = \
            load_preprocessed_data("VAE", source_dataset, dataset_key, hpo_fold_idx,
        ...
        def objective(trial):
            ...
            # Train model
            model_xgb = initialize_train__regressor(X_train, y_train, **params)

            # Predict and evaluate
            y_pred = model_xgb.predict(X_test)
            reg_metrics = evaluate_VAE_model(model_xgb, X_test, y_test, y_pred)
